<?php
/*
Plugin Name: Twitter for Wordpress extended 2
Version: 1.0.3.1
Plugin URI: http://www.andreamoro.co.uk/
Author: Andrea Moro
Author URI: http://www.andreamoro.co.uk/
Description: Displays your tweets/friends timeline/search results/twitter trends in your WordPress blog. This plug-in is a fork of http://www.das-motorrad-blog.de/meine-wordpress-plugins/, which is based on <a href="http://rick.jinlabs.com/code/twitter">Twitter for Wordpress</a> by <a href="http://rick.jinlabs.com/">Ricardo González</a>.
License: GPL
*/
...
$twitter_options[ 'widget_fields' ][ 'titlewidgetlink' ] = array( 'label' => 'Widget title will contain a link:', 'type' => 'checkbox', 'default' => 'false' );
...
$twitter_options[ 'widget_fields' ][ 'nofollowattribute' ] = array( 'label' => 'Nofollow links:', 'type' => 'checkbox', 'default' => true );
...
$twitter_options[ 'widget_fields' ][ 'excludeRT' ] = array( 'label' => 'Exclude Re-tweets:', 'type' => 'checkbox', 'default' => true );
...
$twitter_options[ 'widget_fields' ][ 'excludeText' ] = array( 'label' => 'Exclude tweets that contain the following text', 'type' => 'text', 'default' => '' );
...
function twitter_friends( $username = '', $password = '', $num = 10, $list = false, $update = true, $hyperlinks = true, $nofollowattribute = true, $twitter_users = true, $encode_utf8 = false, $excludeRT = true, $excludeText = '' )
{
	global $twitter_options;
	...
	else
	{
		...
		else
		{
			...
			foreach( $messages -> items AS $message )
			{
				...
				$msg = parse_text_for_exclusion($msg, $excludeText);
				if (strlen($msg) == 0)
					continue;				
				...
				if( $list )
					...
					if( $hyperlinks )
						$msg = hyperlinks( $msg, $nofollowattribute );
...
function twitter_searchstring( $searchstring = '', $num = 10, $list = false, $imgwidth = 32, $update = true, $hyperlinks = true, $nofollowattribute = true, $twitter_users = true, $encode_utf8 = false, $excludeRT = true, $excludeText = '' )
{
	global $twitter_options;
...
// Display Twitter messages
function twitter_messages( $username = '', $num = 10, $list = false, $update = true, $linked = '#', $hyperlinks = true, $nofollowattribute = true, $twitter_users = true, $encode_utf8 = false, $excludeRT = 'false', $excludeText = '' )
{
	global $twitter_options;
	...
	else
	{
		...
		else
		{
			...
			foreach( $messages -> items AS $message )
			{
				...
				{
					if( $linked == 'all' )
						$msg = '<a href="' . $link . '" ' . ($nofollowattribute != false) ? ' nofollow="true" ' : '' . 'class="twitter-link">' . $msg . '</a>';
					else
						$msg = $msg . '<a href="' . $link . '" ' . ($nofollowattribute != false) ? ' nofollow="true" ' : '' . 'class="twitter-link">' . $linked . '</a>';
				}
...
// Link discover stuff
function hyperlinks($text, $nofollowattribute = false) {
	...
	if ($nofollowattribute != false)
		$text = str_replace('href=', 'nofollow="true" href=', $text);
...
function parse_text_for_exclusion($msg, $excludeText)
{
	if (strlen($msg) != 0){
		if (is_array($excludeText)){
			foreach ($excludeText as $v) {
				$pattern = '/\.*' . $v . '\.*/i';
		...
		{
			$pattern = '/^array\(/i';
			if (preg_match($pattern, $excludeText)){
				$excludeText = substr($excludeText, 6);
				$excludeText = str_replace(';', '', $excludeText);
				$excludeText = str_replace(')', '', $excludeText);
				$excludeText = str_replace('\'', '', $excludeText);
				$excludeText = split(',', $excludeText);
...
function widget_twitter_init()
{
	...
	function widget_twitter( $args, $number = 1 )
	{
		...
		if ($titlewidgetlink)
		 	echo ('<a href="http://twitter.com/' . $item[ 'username' ] . '" class="twitter_title_link">');
		...
		if( $item[ 'display_what' ] == 'currenttrends' )
			...
			if( $item[ 'display_what' ] == 'friendstimeline' && !empty( $item[ 'password' ] ) )
				twitter_friends( $item[ 'username' ], $item[ 'password' ], $item[ 'num' ], true, $item[ 'update' ], $item[ 'hyperlinks' ], $item[ 'twitter_users' ], $item[ 'encode_utf8' ], $item[ 'excludeRT' ], $item[ 'excludeText' ] );
		else
			if( $item[ 'display_what' ] == 'searchresult' && !empty( $item[ 'tag' ] ) )
				twitter_searchstring( $item[ 'tag' ], $item[ 'num' ], true, $item[ 'imgwidth' ], $item[ 'update' ], $item[ 'hyperlinks' ], $item[ 'twitter_users' ], $item[ 'encode_utf8' ], $item[ 'excludeRT' ], $item[ 'excludeText' ] );
		else
			if( $item[ 'display_what' ] == 'usertimeline' && !empty( $item[ 'username' ] ) )
				twitter_messages( $item[ 'username' ], $item[ 'num' ], true, $item[ 'update' ], $item[ 'linked' ], $item[ 'hyperlinks' ], $item[ 'twitter_users' ], $item[ 'encode_utf8' ], $item[ 'excludeRT' ], $item[ 'excludeText' ] );
